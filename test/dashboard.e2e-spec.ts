...
test('display day orders amount metric', async ({ page }) => {
  ...
  expect(page.getByText('20-5% em relação a ontém.')).toBeVisible()
...
test('display month canceled orders amount metric', async ({ page }) => {
  ...
  expect(page.getByText('5-5% em relação a ontém')).toBeVisible()
...
test('display month revenue metric', async ({ page }) => {
  ...
  expect(page.getByText('+10% em relação a ontém.')).toBeVisible()
